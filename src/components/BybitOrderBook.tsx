...
import { useBybitSocket } from "../sockets/useBybitSocket";

export default function BybitOrderBook() {
  const { bids, asks } = useBybitSocket();
  ...
  // Sort asks low → high (best ask at top)
  const sortedAsks = [...asks]
    .map(([p, a]) => [parseFloat(p), a] as [number, string])
    .sort((a, b) => b[0] - a[0]); // descending (highest → lowest)

  // Sort bids high → low (best bid at top)
  ...
  return (
    <div className="p-2 bg-gray-900 rounded-md shadow-md inline-block mx-auto w-[23vw]">
      ...
      <div className="flex items-center justify-center mb-2 gap-1">
        <img
          src="https://upload.wikimedia.org/wikipedia/commons/1/14/Bybit_Logo.svg"
          alt="Bybit Logo"
          className="w-6 h-6 bg-white rounded p-0.5"
        />
        <h2 className="text-sm font-semibold text-white">
          PUMP/USDT (Bybit Perps)
        </h2>
      </div>
